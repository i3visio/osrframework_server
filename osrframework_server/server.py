...
import osrframework_server.lib.configuration as configuration_server


# First, we will check if the configuration files are stored
...
STATIC_FOLDER = pkg_resources.resource_filename('osrframework_server', os.path.join("data", 'static/'))
TEMPLATES_FOLDER = pkg_resources.resource_filename('osrframework_server', os.path.join("data", 'templates/'))
...
def getParser():
    """
    Generates args parser for this module

    Returns:
    --------
        An argparse.ArgumentParser for the current module.
    """
    DEFAULT_VALUES = configuration_server.getConfiguration(
    ...
    groupServerConfiguration.add_argument('--debug', required=False, default=False if DEFAULT_VALUES["debug"] == "0" else True, action='store_true', help='choose whether error messages will be deployed. Do NOT use this for production.')

    groupAbout = parser.add_argument_group('About arguments', 'Showing additional information about this program.')
    groupAbout.add_argument('-h', '--help', action='help', help='shows this help and exists.')
    groupAbout.add_argument('--version', action='version', version='OSRFramework Server ' + osrframework_server.__version__ +  " is running OSRFramework " + osrframework.__version__ + " in this computer.", help='shows the version of the program and exists.')
...
def main(params=None):
    ...
    # Starting the server
    print(general.info("[*] OSRFramework Server " + osrframework_server.__version__ + " started at " + "http://" + args.host + ":" + str(args.port) + "... You can access it in your browser."))
